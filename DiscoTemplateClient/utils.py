...
def get_dict(model_str):
    try:
        found_model = apps.get_model("DiscoTemplateClient", model_str)
    except LookupError:
        return {}
    try:
...
def export_data(choices):
    ...
    if "Configuration" in choices:
        models_to_dump.append("DiscoTemplateClient.Configuration")
    if "EventLog" in choices:
        models_to_dump.append("DiscoTemplateClient.EventLog")
        models_to_dump.append("DiscoTemplateClient.ErrLog")
    if "User" in choices:
        models_to_dump.append("DiscoTemplateClient.User")
        models_to_dump.append("DiscoTemplateClient.DiscordServer")
